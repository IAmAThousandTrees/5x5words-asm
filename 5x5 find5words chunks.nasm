...
;; working with data locality may improve this and subsequent process times, especially the convert2codes
;; which is currently a random-ish gather-from-L3 process.
;; break up the file read into 32k chunks so that chunk and the processing of it can fit into L1 cache
;; tag on the gather within this stage and it's slowness should be reduced.
;; it might make sense to abandon the file after reading and reference the gathered qwords instead of the original file.
...
.chunkloop:
		mov			r11, -4096				; qwords in a chunk - negative loopcounter
		sub			rdx, r11
		lea			r10, [rdi + rdx * 8]	; chunk endpointer
.charLoop:
		vmovdqa32	zmm0, [r10 + r11 * 8]	; aquire bytes
		vpsubb		zmm0, zmm0, zmm20		; A=0
		vpandnd		zmm0, zmm21, zmm0		; a=A=0
		vpcmpnltub	k1, zmm0, zmm22			; letter=false
		kmovq		[r8 + r11], k1			; store results
		add			r11, 8
...
; to move from byte based to dword based addressing is another 2 bits reduction. convert to bottom adr, round up length, convert to top adr
		sub			r8, 4096				; back to startpointer
